...
# use-after-free fixed in pistonball_replay.nim (emscripten_exit_with_live_runtime)
...
# pistonball_error_len(). Aborting keeps linear memory intact, and the page reads
# pistonball_stage_ptr/len afterwards to report what the runtime was doing.
switch(
  "passL",
  (&"""
  -o {distDir / "pistonball_replay.js"}
  ...
  -s EXPORTED_FUNCTIONS=_main,_malloc,_free,_pistonball_load_replay,_pistonball_frame,_pistonball_input,_pistonball_packet_ptr,_pistonball_packet_len,_pistonball_mismatch_tick,_pistonball_error_ptr,_pistonball_error_len,_pistonball_stage_ptr,_pistonball_stage_len
  """).replace("\n", " ")
)
