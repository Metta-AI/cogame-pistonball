...
# Substitute: pistonball, coworld-pistonball, 20.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-pistonball:ci)
#   SMOKE_SLUG                 game slug                        (pistonball)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/pistonball)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/pistonball-player)
#   SMOKE_MANIFEST             manifest template path           (coworld_manifest_template.json)
#   SMOKE_SEATS                seat-count CROSS-CHECK           (20)
...
#   ANTHROPIC_API_KEY          if set, forwarded to player containers for
#                              local prompt-policy smoke
set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_dir="$(cd "${script_dir}/../.." && pwd)"

image="${1:-${SMOKE_IMAGE:-coworld-pistonball:ci}}"
slug="${SMOKE_SLUG:-pistonball}"
